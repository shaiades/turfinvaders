...
import { backSolveVolumeGoal, projectedPlaybookTakeHome, resolveRepRates } from "@/lib/rep-goals";
import { laTodayISO, remainingWorkdaysInWeek } from "@/lib/dates";
import type { KombatTotals, RepStats } from "@/lib/close-kombat";
...
export function CloseKombatGoalsTab({
  ...
  /** Company-wide totals over the same trailing window (UNFILTERED by the
   *  office picker) — the fallback when the rep's own history is too thin. */
  trailingCompanyTotals: KombatTotals | null;
  ...
  const met = goal !== null && goal > 0 && volume >= goal;

  const resolved = resolveRepRates(trailingRow, trailingCompanyTotals);
  const solve =
    goal && goal > 0
      ...
      <div className="space-y-5">
          ...
          <div className="rounded-lg border border-kombat-gold/40 bg-[color-mix(in_oklab,var(--kombat-gold)_7%,var(--surface))] p-5">
            <div className="flex items-baseline justify-between gap-2">
              ...
              <div className="text-[10px] text-muted-foreground">
                {weekLoading ? "Counting…" : `${sales} sale${sales === 1 ? "" : "s"} banked`}
              </div>
            </div>
            ...
            {/* PROJECTED take-home — playbook math only (~20% of ~40% profit
                ≈ 8% of volume), the piggy-bank pattern. Never payroll. */}
                ...
                <div className="mt-2 flex items-baseline justify-between gap-2 flex-wrap text-[10px]">
                  ...
                  {daysLeft > 0 ? (
                    <span className="text-muted-foreground tabular-nums">
                      ...
                      {Math.ceil(solve.apptsNeeded / daysLeft) === 1 ? "" : "s"}/day over the next{" "}
        ...
        <div className="space-y-1">
          ...
          <p className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
            ...
            costs fewer doors knocked on your behalf.
          </p>
        </div>
      </div>
    </ArcadePanel>
