...
/** ── Office-generated appointments (owner, restated 2026-08-31) ───────────
 *  "A canvasser only earns credit for a lead they personally generated by
 *  knocking." The office books its own appointments — job walks, upsells,
 *  reloads, check pickups — and marks them on the Iss column, the board's
 *  own answer to "was a lead issued for this card". Blank or "Iss" means an
 *  issued lead; only an explicit office mark pulls a card out of the
 *  canvasser numbers. The label truncates to "Office A…" on the board, so
 *  match any "office" variant.
 *  ONE signal on purpose: a Sale cell reading "Upsell" (a knocked lead that
 *  upsold at the door) and a Canvass Stats "Reset" (the canvasser's own
 *  reset) are real production and keep their credit.
 *  Write-side twin of isOfficeAppt in src/lib/close-kombat.ts — the
 *  sales-rep read model has excluded these from every lead number since
 *  2026-07-29, so both sides now agree about a card. */
export function isOfficeIss(iss: string | null | undefined): boolean {
  return /office/i.test(iss ?? '')
}

/** The Iss column by exact title — the same lookup buildBlockCardRow uses,
 *  so the credit gate and the snapshot can never disagree. A board that
 *  renames the column yields undefined, and the gate then fails OPEN
 *  (today's behavior) rather than stripping credit from real production. */
export function findIssCol(cols: MondayCol[]): MondayCol | undefined {
  return cols.find((c) => (c.column?.title || '').trim().toLowerCase() === 'iss')
}
