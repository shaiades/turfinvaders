-- Every result counts as a knock (owner directive 2026-09-10, D2DU-video
-- parity): the rep taps the RESULT of the door — there is no separate "Log
-- Knock" step anymore — so every valid pin now bumps doors_knocked, and every
-- conversation-implying result bumps people_talked_to.
--
-- Contribution matrix (valid, non-remote pins):
--   doors_knocked    ALL pin types (knock, not_home, go_back, talked_to,
--                    lead, renter, not_interested, appt)
--   people_talked_to talked_to, renter, go_back (as before) + lead,
--                    not_interested, appt (new — each implies a conversation)
--   not_interested   not_interested (unchanged)
--   leads_called_in  lead (unchanged)
--   renters          renter (unchanged)
-- Appt pins now count the door + the talk, but appointment/sale COUNTS still
-- come from Monday.com, never from pins (owner directive 2026-08-15).
--
-- ROLL FORWARD ONLY — no backfill from pre-existing pins: until today reps
-- tapped "Log Knock" (knock pins) alongside their result pins, so counting
-- doors for historical lead/NI/appt pins would double-count those days.
-- Everything else matches 20260910200000: corrections adjust by delta,
-- remote drops contribute zero on every op, decrements floor at 0 (GREATEST)
-- and never create a daily_logs row.
