-- Pacific-time bucketing (owner directive 2026-07-20): ALL date/week buckets
-- reflect America/Los_Angeles, never UTC. The pay week runs Mon–Sat with all
-- stats resetting at midnight PT Monday morning.
--
-- What changes:
--   1) calc_weekly_paycheck / calc_monthly_paycheck: a confirmed sale's week
--      is derived from COALESCE(reviewed_at, created_at) in LA time (was UTC —
--      sales landing after 5pm PT slid into the next UTC day, and late-Sunday
--      sales into the wrong pay week).
--   2) refresh_canvasser_rank / evaluate_canvasser_suspension: "this week"
--      anchors on the LA Monday (was UTC CURRENT_DATE, which flips to Monday
--      at 4/5pm PT Sunday).
--   3) daily_logs / time_entries / field_pins log_date defaults stamp the LA
--      calendar date (daily_metrics.metric_date already did).
-- Note: date_trunc('week', ts) is ISO — Monday start — matching the pay week.
-- The webhook writers that pin sale reviewed_at to 20:00Z (edge function) or
-- 12:00Z (CSV import) still bucket correctly: both instants fall inside the
-- same LA calendar day.

-- ── 1a) calc_weekly_paycheck v5: LA-time sale-week attribution ──────────────
--        Identical to v4 (20260718192842) except the leads ::date cast.
...
-- ── 1b) calc_monthly_paycheck: LA-time sale-month attribution ───────────────
--        Identical to 20260629174006 except the leads ::date cast.
...
-- ── 2a) refresh_canvasser_rank: LA-time week anchor ─────────────────────────
--        Identical to 20260718183328 except _this_week derives from LA now().
...
DECLARE
  ...
  _rolling4 numeric := 0;
  _recruits int := 0;
  _this_week date := (date_trunc('week', now() AT TIME ZONE 'America/Los_Angeles'))::date;
  _wk date;
  _wk_sits int;
  _rank text;
  _stored_rank text;
...
-- ── 2b) evaluate_canvasser_suspension: LA-time week anchor ──────────────────
--        Identical to 20260630004105 except the week anchor.
...
AS $function$
DECLARE
  _this_week_start date := (date_trunc('week', now() AT TIME ZONE 'America/Los_Angeles'))::date;
...
-- ── 3) Write-side date defaults stamp the LA calendar date ──────────────────
--       (daily_metrics.metric_date already defaults to LA.)
ALTER TABLE public.daily_logs
  ALTER COLUMN log_date SET DEFAULT (now() AT TIME ZONE 'America/Los_Angeles')::date;
