...
const DEST_DB_URL = process.env.DEST_DB_URL
  || readFileSync(join(homedir(), '.turf-dest-db-url'), 'utf8').trim()
const PSQL = '/opt/homebrew/opt/postgresql@18/bin/psql'

function sqlRows(query) {
  const out = execFileSync(PSQL, [DEST_DB_URL, '-At', '-v', 'ON_ERROR_STOP=1', '-c',
...
function sqlExecFile(path) {
  execFileSync(PSQL, [DEST_DB_URL, '-v', 'ON_ERROR_STOP=1', '-q', '-f', path],
    { encoding: 'utf8', stdio: ['ignore', 'inherit', 'inherit'], env: { ...process.env, LC_ALL: 'C' } })
}
...
async function monday(query, variables) {
  ...
  if (json.errors) die(`Monday API: ${JSON.stringify(json.errors).slice(0, 500)}`)
  if (json.error_message) die(`Monday API: ${json.error_message}`)
  return json.data
}
...
function mapScheduleOutcome(columnTitle, value) {
  ...
  if (!v) return null
  if ((col === 'bo' || col.includes('blowout')) && (v === 'no show' || v === 'no demo' || v === 'bo')) return 'blowouts'
  if ((col === 'ol' || col.includes('outside lead')) && (v === 'ol' || v === 'outside lead')) return 'outside_leads'
  if ((col === 'rs' || col.includes('reset')) && (v === 'reset' || v === 'rs')) return 'resets'
  if ((col === 'pm' || col.includes('pitch missed')) && (v === 'pm' || v.startsWith('pm'))) return 'pitch_missed'
  if ((col === 'sale' || col.includes('sale')) && ['sold', 'reload', 'upsell', 'sale'].includes(v)) return 'sales'
...
const OUTCOME_PRIORITY = ['sales', 'pitch_missed', 'resets', 'blowouts', 'killed', 'outside_leads', 'leads_confirmed', 'no_answers', 'pending']
...
const LOG_KEYS = ['no_demo', 'one_legs', 'future_leads', 'demos_sits', 'sales', 'confirmed_leads', 'unmarked']
...
const findAgentCol = (cols) => cols.find((c) => { const t = norm(c.column?.title); return t === 'agent' || t.includes('agent') || t.includes('canvasser') })
...
async function fetchBoardItems(boardId) {
  ...
  while (page.cursor) {
    data = await monday(`query { next_items_page(cursor: ${JSON.stringify(page.cursor)}, limit: 100) {
      cursor items { id name created_at column_values { id text column { id title } } } } }`)
...
function mapItem(item) {
  ...
  const date = parseDateText(dateCol?.text) || (item.created_at ? String(item.created_at).slice(0, 10) : null)
  const found = new Set()
  for (const c of cols) {
...
function matchProfile(profiles, agentName) {
  ...
  return profiles.find((p) => p._norm === wanted)
    ?? profiles.find((p) => p._norm.includes(wanted))
    ?? profiles.find((p) => wanted.includes(p._norm))
    ?? (wanted.split(' ')[0] ? profiles.find((p) => p._norm.split(' ')[0] === wanted.split(' ')[0]) : undefined)
}

// ── commands ────────────────────────────────────────────────────────────────
async function cmdListBoards() {
  const data = await monday('query { boards(limit: 100, order_by: created_at, state: all) { id name state items_count } }')
...
const EDGE_URL = 'https://xogitpqeuwalerxygvjw.supabase.co/functions/v1/monday-live-dispatch?apikey=sb_publishable_ivjX0mrVvSLM1DHfDTDVuw_qHUtGeS2'
...
async function registerOn(boardId) {
  ...
  for (const event of ['create_item', 'change_column_value']) {
    ...
    out.push({ board_id: String(boardId), webhook_id: String(wh.id), event, registered_at: new Date().toISOString() })
    console.log(`registered ${event} on board ${boardId} -> webhook ${wh.id}`)
  }
  const patch = q(JSON.stringify(out))
  execFileSync(PSQL, [DEST_DB_URL, '-v', 'ON_ERROR_STOP=1', '-qc',
...
async function cmdRegisterActive() {
  const s = sqlRows('SELECT active_monday_board_oc, active_monday_board_sd FROM public.system_settings LIMIT 1')[0]
  if (!s) die('no system_settings row')
  for (const [label, id] of [['OC', s.active_monday_board_oc], ['SD', s.active_monday_board_sd]]) {
    if (!id) { console.log(`(${label}: no active board id set — skipped)`); continue }
...
async function cmdCreateTestItem(boardId, agent, name) {
  ...
  const agentCol = (data.boards?.[0]?.columns || []).find((c) => {
    const t = norm(c.title); return t === 'agent' || t.includes('agent') || t.includes('canvasser')
  })
  ...
  console.log(`created item ${created.create_item.id} on board ${boardId} (agent col: ${agentCol?.id ?? 'NONE'})`)
}
...
async function cmdBackfill() {
  const boards = String(args.boards || '').split(',').map((s) => s.trim()).filter(Boolean)
  const since = args.since, until = args.until
  if (!boards.length || !/^\d{4}-\d{2}-\d{2}$/.test(since || '') || !/^\d{4}-\d{2}-\d{2}$/.test(until || ''))
    die('usage: backfill --boards a,b --since YYYY-MM-DD --until YYYY-MM-DD [--apply] [--create-agents]')
  ...
  console.error(`# ${all.length} items total, ${inSpan.length} in span, ${skippedNoAgent.length} skipped (no agent)`)
  ...
  const resolve = () => {
    ...
    for (const r of rows) {
      ...
      if (m) { r.canvasser_id = m.id; r.team_id = m.team_id; r.office = m.office_location }
  ...
  for (const r of rows) {
    ...
    if (!logs.has(key)) logs.set(key, { canvasser_id: r.canvasser_id, team_id: r.team_id, log_date: r.date,
      no_demo: 0, one_legs: 0, future_leads: 0, demos_sits: 0, sales: 0, confirmed_leads: 0, unmarked: 0 })
  ...
  const curLeads = sqlRows(`SELECT count(*)::int AS n, COALESCE(sum(sale_amount),0)::numeric AS total
  ...
  for (const a of [...logs.values()].sort((x, y) => (nameOf.get(x.canvasser_id) || '').localeCompare(nameOf.get(y.canvasser_id) || '') || x.log_date.localeCompare(y.log_date)))
    console.log(`${nameOf.get(a.canvasser_id)}\t${a.log_date}\t${a.demos_sits}\t${a.sales}\t${a.future_leads}\t${a.no_demo}\t${a.one_legs}\t${a.confirmed_leads}\t${a.unmarked}`)
  console.log(`\n--- would write: ${sales.length} confirmed sale leads (total $${sales.reduce((s, r) => s + (r.salePrice ?? 0), 0)}) ---`)
  for (const s of sales) console.log(`${nameOf.get(s.canvasser_id)}\t${s.date}\t$${s.salePrice ?? 'MISSING'}\t${s.name}\titem=${s.id}`)
  ...
  for (const id of ids)
    execFileSync(PSQL, [DEST_DB_URL, '-Atqc', `SELECT public.refresh_canvasser_rank(${q(id)})`], { encoding: 'utf8' })
  ...
  console.log(dups.length ? `!!! DUPLICATE monday_item_id rows: ${JSON.stringify(dups)}` : 'duplicate guard clean ✓')
}
...
const run = {
  ...
  'register-webhooks': async () => { for (const b of String(args.boards || '').split(',').filter(Boolean)) await registerOn(b) },
